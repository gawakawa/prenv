...
After adding or updating dependencies, update the `fetchPnpmDeps` hash in `../flakes/checks.nix` (repo root):
1. Run `nix flake check` from the repo root — it will fail with a hash mismatch error
