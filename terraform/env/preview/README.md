...
| <a name="input_commit_sha"></a> [commit\_sha](#input\_commit\_sha) | Full Git commit SHA deployed, passed in by CI. Empty on teardown, where no build happens. Exposed to the backend container so the dashboard can link to the exact commit — image tags are content-hashes, not commit SHAs. | `string` | `""` | no |
